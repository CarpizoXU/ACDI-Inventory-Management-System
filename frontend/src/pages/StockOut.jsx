  ...
  async function loadData() {
    try {
      ...
      const [productsResponse, transactionsResponse] = await Promise.all([
        ...
        transactionService.getTransactions({ type: 'stock-out', limit: 5 }),
      ]);
  ...
  return (
    <div>
      <div className="mb-6 grid gap-6 xl:grid-cols-[1.5fr_1fr]">
        ...
        <div className="rounded-3xl bg-white p-6 shadow-sm">
          <div className="mb-5">
            <h2 className="text-xl font-semibold text-slate-900">Issue Items</h2>
            <p className="mt-2 text-sm text-slate-500">Fill in the details below to issue stock.</p>
          </div>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={saving}
              className="inline-flex w-full items-center justify-center rounded-2xl bg-rose-600 px-4 py-3 text-sm font-semibold text-white hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-70"
            >
